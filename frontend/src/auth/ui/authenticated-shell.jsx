    ...
    <div className="authenticated-app">
      ...
      <div className="authenticated-layout">
        <aside id="authenticated-sidebar" className={`authenticated-sidebar${drawerIsVisible ? " is-open" : ""}`} aria-label="Điều hướng chính" aria-hidden={isMobileNavigation && !isDrawerOpen} inert={isMobileNavigation && !isDrawerOpen || undefined}>
          <nav className="authenticated-navigation" aria-label="Khu vực học tập"><NavLink to="/dashboard" end onClick={() => closeDrawer({ restoreFocus: false })} className={({ isActive }) => `authenticated-nav-link${isActive ? " is-active" : ""}`}><BookOpenCheck className="size-5" aria-hidden="true" /><span>Dashboard</span></NavLink>{user.role === "ADMIN" ? <><NavLink to="/admin/topics" onClick={() => closeDrawer({ restoreFocus: false })} className={({ isActive }) => `authenticated-nav-link${isActive ? " is-active" : ""}`}><Tags className="size-5" aria-hidden="true" /><span>Quản lý chủ đề</span></NavLink><NavLink to="/admin/vocabulary" onClick={() => closeDrawer({ restoreFocus: false })} className={({ isActive }) => `authenticated-nav-link${isActive ? " is-active" : ""}`}><BookText className="size-5" aria-hidden="true" /><span>Quản lý từ vựng</span></NavLink></> : null}</nav>
